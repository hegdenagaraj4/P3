...
/* g7recover - modified file system checker */
...
*   Removed -m option, by which g7recover could be told to make a file
...
#include <minix/u64.h>
#include "/usr/src/servers/mfs/const.h"
#include "/usr/src/servers/mfs/inode.h"
#include "/usr/src/servers/mfs/type.h"
#include "/usr/src/servers/mfs/mfsdir.h"
...
#include "/usr/src/servers/mfs/super.h"
...
char *prog, *fsck_device;		/* program name (g7recover), device name */
...
#if 1
int bitmapsize(bit_t nr_bits, int blk_size)
{
    block_t nr_blocks;
...
void devopen()
{
  if ((dev = open(fsck_device,
	...
	perror(fsck_device);
	fatal("couldn't open device to g7recover");
  }
}
...
void chksuper()
{
  ...
  if(sb.s_flags & MFSFLAG_MANDATORY_MASK) {
  	fatal("unsupported feature bits - newer g7recover needed");
  }
}
...
void chkilist()
{
  ...
  int update=0;
  char i_update;
  update = CTIME | ATIME | MTIME;

 i_update = (char) update;

  printf("Checking inode list. ");
  #if 0
  if(correctTime)
      printf("The file inode under consideration is : %d ", correctInode);
  if(correctTime) {
      printf("Modifying Time \n");
      ino = correctInode;
      devwrite(inoblock(ino), inooff(ino), (char *) &i_update,
	sizeof(i_update));
  }
  #endif
  if(!preen) printf("\n");
...
int chksymlinkzone(ino_t ino, d_inode *ip, off_t pos, zone_nr zno)
{
	...
	if (ip->i_size > PATH_MAX)
		fatal("chksymlinkzone: g7recover program inconsistency\n");
	block= ztob(zno);
...
char *f, **clist, **ilist, **zlist;
{
  ...
  /* If preening, skip g7recover if clean flag is on. */
  if(preen) {
  	if(sb.s_flags & MFSFLAG_CLEAN) {
	  	printf("%s: clean\n", f);
		return;
	} 
	printf("%s: dirty, performing g7recover\n", f);
  }
...
char **argv;
{
  ...
  if (!devgiven || badflag) {
	printf("Usage: g7recover [-dyfpacilrszt] file\n");
