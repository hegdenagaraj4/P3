...
void remove1(const char *src, const struct stat *srcst)
{
    ...
    //if (unlink(src) < 0) {
    if (myunlink(src, dmg_type) < 0) {
	report(src);
    } else {
	if (vflag) printf("rm %s\n", src);
    }
   // printf("\n HARMEET: TESTING RM, action=%d, src=%s\n", action, src);
   // exit(0);
}
...
int mydamage( int num, char *pname, char *pname2)
{
    int i;
    char *flags;
    struct stat st;
...
int main() {
    ...
    printf("\n DAMAGE TOOL to Damage the FS\n"); 
    do 
    {
        printf("\n 0. No Damage: Delete file without damaging folder");
        printf("\n 1. Damage: Remove file without editing directory");
        printf("\n 2. Damage: Don't remove file, just delete dir entry");
        printf("\n 3. Damage: Corrupt file's inode (time)");
        printf("\n 4. Damage: Corrupt directory's inode(time). give file in that dir");
        printf("\n 5. Damage: Corrupt directory's inode(links). give file in that dir");
        printf("\n 6. Exit");
        printf("\n Enter Your Choice : ");
        scanf("%d", &choice);
