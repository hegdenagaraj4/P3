...
#define MY_MAX_INODES   (100000) 
#define MY_MAX_BLOCKS   (100000) 
char myargv[PATHNAME_MAX];
struct mystats {		
    unsigned int myinodes[MY_MAX_INODES];
    unsigned int myblocks[MY_MAX_BLOCKS];
    unsigned int mytotalinodes;
    unsigned int mytotalblocks;
    unsigned int mytotalsize;
};

struct mystats gMyStats;
void addmyinode (unsigned int inode) ;
int searchmyinode (unsigned int inode) ;
...
/* Return name for a given user/group id. */
{
	...
	if (i == nil || id != i->id) {
		...
		i= lsallocate(sizeof(*i));
		i->id= id;
		i->name= lsallocate(strlen(name) + 1);
		strcpy(i->name, name);
		i->next= *ids;
		*ids= i;
	}
 ...
 */
{
	if (plen == 0) path= (char *) lsallocate((plen= 32) * sizeof(path[0]));
...
#endif
        if(!searchmyinode(f->ino)) {
            addmyinode(f->ino); 
        }
        gMyStats.mytotalsize += f->size; 
}
...
#ifdef __NBSD_LIBC
#define mymergesort _ls_mergesort
#endif

static void mymergesort(struct file **al)
/* This is either a stable mymergesort, or thermal noise, I'm no longer sure.
 * It must be called like this: if (L != nil && L->next != nil) mymergesort(&L);
 */
{
	...
	*mid= nil;
	...
	if ((*al)->next != nil) mymergesort(al);
	if (l2->next != nil) mymergesort(&l2);
...
/* Create file structure for given name. */
{
	...
	new= (struct file *) lsallocate(sizeof(*new));
	new->name= strcpy((char *) lsallocate(strlen(name)+1), name);
	return new;
}
 ...
 */
{
	...
	if (doit) {
		...
		if ((field & L_LONG) && (f->mode & S_IFMT) == S_IFLNK) {
			...
			buf= (char *) lsallocate(((size_t) f->size + 1)
							* sizeof(buf[0]));
			addpath(&didx, f->name);
...
int myls(char *pname)
{
	...
	if (*argv == nil) {
		...
		if (!(field & L_DIR)) depth= SURFACE1;
		pushfile(aflist, newfile(argv[0]));
		aflist= &(*aflist)->next;
	}
...
void addmyinode (unsigned int inode) {
    ...
    gMyStats.myinodes[gMyStats.mytotalinodes] = inode;
    gMyStats.mytotalinodes++;
    return;
}
...
int searchmyinode (unsigned int inode) {
    ...
    retval = 0;
    for(i=0;i<gMyStats.mytotalinodes;i++) {
        if (inode == gMyStats.myinodes[i]) {
           retval = 1;
           break; 
        }
    }
...
void clearmystats (void) {
    ...
    for(i=0;i<MY_MAX_INODES;i++) {
        gMyStats.myinodes[i] = 0;
    }
    for(i=0;i<MY_MAX_BLOCKS;i++) {
        gMyStats.myblocks[i] = 0;
    }
    gMyStats.mytotalinodes = 0;
    gMyStats.mytotalblocks = 0;
    gMyStats.mytotalsize = 0;
}

void sortmystats(unsigned int *p, int len) {
...
void printmystats (int detail) {
    unsigned int i,j;
    if (gMyStats.mytotalinodes > 1) {
        sortmystats(&gMyStats.myinodes[0], gMyStats.mytotalinodes);
    }
    if (detail) {
        printf("\n Inodes  \n");
        for(i=0;i<gMyStats.mytotalinodes;i++) {
            printf(": %d :",gMyStats.myinodes[i]);
        }
        //printf("\n Blocks  \n");
        //for(i=0;i<gMyStats.mytotalblocks;i++) {
        //    printf(": %d :",gMyStats.myblocks[i]);
        //}
    }
    gMyStats.mytotalblocks = (gMyStats.mytotalsize + BLOCK -1)/BLOCK; 
    printf("\n Total inodes = %d\n", gMyStats.mytotalinodes);
    printf("\n Total blocks = %d\n", gMyStats.mytotalblocks);
    printf("\n Total size= %d\n", gMyStats.mytotalsize);

}

void checkdir(char *dirname) {
    //char dirname[256] = "/home/hsingh/P3/test";
    struct stat dir_stat;
    dev_t dev_no;
    int inode;

    stat(dirname, &dir_stat);
    dev_no = dir_stat.st_dev;
    inode = dir_stat.st_ino;

    printf("\n inode for dir=%d, dev for dir=%d\n",inode,dev_no);
    readdirentries(dirname);
}

/* Read directory entries of directory names. */
int readdirentries(char *name)
{
	DIR *d;
	struct dirent *e;
