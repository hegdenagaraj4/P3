...
int main(int argc, char **argv)
{
	 ...
	 */
    printf("\n Harmeet:4: i=%d, argc=%d argv[argc-1]=%s\n", i, argc, argv[argc-1]);
	path_add(&dst, argv[argc-1]);
	slash= path_length(&dst);

	do {
    printf("\n Harmeet:5: i=%d, argv[i]=%s\n", i, argv[i]);
	    path_add(&src, argv[i]);
